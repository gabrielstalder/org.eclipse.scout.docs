...
Finally, add the SDK related maven dependencies to the maven module(s), e.g. the server and ui.html, according to <<lst-contacts_db_pom>>.
...
or by providing an implementation of the interface `org.eclipse.scout.rt.opentelemetry.api.IMetricProvider`.
...
TIP: It is possible to define specific link:https://opentelemetry.io/docs/specs/otel/metrics/sdk/#explicit-bucket-histogram-aggregation[explicit bucket distribution] for histogram metrics, see `org.eclipse.scout.rt.opentelemetry.api.IHistogramViewHintProvider`
